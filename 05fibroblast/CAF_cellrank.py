...
data = readRDS("05FIBdataset.res1.rds")
meta = read.csv("metadata.34207.csv",header=T,row.names=1)
meta1 = read.csv("allcell_meta.csv",header=T,row.names=1)

test = data@reductions$umap@cell.embeddings
test2 = meta1[rownames(meta),]
test2$seurat_cluster = meta$fb_cluster
test2$celltype = meta$fb_celltype
result = cbind(test,test2)
write.csv(result,"cellrank/fb_embedding.csv")
...
meta_path = "/home/public/myspace/daijiacheng/03GeosAltas/05fibroblast/cellrank/"
...
adata.write("/home/public/myspace/daijiacheng/03GeosAltas/05fibroblast/cellrank/dataset_dynamicModel.h5ad",compression='gzip')
...
adata = scv.read('/home/public/myspace/daijiacheng/03GeosAltas/05fibroblast/cellrank/dataset_dynamicModel.h5ad')
...
cr.tl.initial_states(adata, cluster_key="celltype",n_states=5)
...
cr.pl.lineage_drivers(adata, lineage="Fib.C20.MYOC", n_genes=8)

#gene expression trends
root_idx = np.where(adata.obs["initial_states"] == "Fib.C8.SCN7A")[0][0]
...
cr.pl.heatmap(adata, model, genes=adata.var['to CAF.PLA2G2A corr'].sort_values(ascending=False).index[:100], show_absorption_probabilities=False, lineages="CAF.PLA2G2A", n_jobs=16, backend="loky")
